...
tests =
    describe "triangleKind"
        ...
        , test "degenerate triangles are illegal" <|
            \() -> Expect.equal (Err "Degenerate triangle") (triangleKind 1 1 2)
        , fuzz (Fuzz.intRange 1 Random.maxInt) "degenerate integer triangles are illegal" <|
            \i -> Expect.equal (Err "Degenerate triangle") (triangleKind i i (i + i))
        , fuzz (Fuzz.floatRange 1.0e-15 1.0e100) "degenerate float triangles are illegal" <|
            \f -> Expect.equal (Err "Degenerate triangle") (triangleKind f f (f + f))
        ]
